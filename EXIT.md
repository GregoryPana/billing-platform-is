...
- [ ] **Blocking:** no `BILLING_ENTRA_*` GitHub Actions secrets exist yet (`gh secret list` confirms only non-Entra secrets are set). The deploy job defaults every `ENTRA_*`/`VITE_ENTRA_*` var to empty/`false` until these are added. Only `BILLING_ENTRA_ENABLED`, `BILLING_ENTRA_TENANT_ID`, `BILLING_ENTRA_CLIENT_ID` (backend) and `BILLING_ENTRA_TENANT_ID`, `BILLING_ENTRA_CLIENT_ID` (frontend, reused) are strictly required — `authority`/`issuer`/`audience`/`jwks_url`/`redirect_uri`/`api_scope` all have safe derived defaults in `entra_auth_service.py` and `frontend/src/entra.js` if left unset. Values are recorded in the registration table above. Gregory to add via `gh secret set` or GitHub Settings UI.
