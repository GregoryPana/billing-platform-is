...
`DESIGN_SYSTEM.md:87-91` mandates `recharts` (charts), `framer-motion` (mount/unmount + state feedback), `react-hook-form` + `zod` (nontrivial forms), and `@tanstack/react-table` (sortable/paginated tables). None of the four are present in `frontend/package.json` as of this rebaseline.
...
| `react-hook-form` + `zod` | No | Open backlog item | Tracked as **Package 8 — "React Hook Form and Zod hardening"** in `docs/RG01_ALIGNMENT_TRACKER.md` (queued, not yet started). |
| `@tanstack/react-table` | No | Open backlog item | Tracked as **Package 9 — "TanStack Audit/Reporting data tables"** in `docs/RG01_ALIGNMENT_TRACKER.md` (queued, not yet started). |
...
| Forms | Manual `useState`-based forms | No `react-hook-form`/`zod` — see Package 8 above. |
| Tables | Hand-rolled / shadcn-style tables | No `@tanstack/react-table` — see Package 9 above. |
