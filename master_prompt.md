...
**1. Cloud Transfer:** Move finalized logic from local testing to the production runtime environment.
...
## 🧭 CI/CD & Repo Standards (Mandatory, Enforced)

### 1. GitHub Repo Setup
- Initialize a git repo and push a baseline `main` branch.
- Never commit secrets; use `.env.example` + GitHub Secrets.
- Keep environment-specific files out of version control.

### 2. GitHub Actions Workflow (Enforced)
- You must provide CI workflows that run tests and build steps.
- Deploy only after CI passes (`needs: test`).
- Include a clean deployment script (git pull + container restart or service restart).
- Deployment jobs must emit visibility metadata (app name, version/commit hash, deploy timestamp, environment, health status).

### 3. Self-Hosted Runner (Preferred)
- Use a self-hosted runner for private VMs.
- Install under `/opt/actions-runner` and run as a service.
- Ensure the runner uses repo-local deploy commands (no hardcoded secrets).

### 4. Deploy-Time Environment
- Generate `.env` files from GitHub Secrets at deploy time.
- Never rely on manual env edits on the VM.
- Prefer Docker Compose for repeatable deployments.
- Emit deployment visibility metadata as a build/deploy artifact or log line.

### 5. Reliability Defaults
- Add retries for external pulls (Docker Hub) when needed.
- Avoid interactive steps inside CI.
- For immutable deploys, prefer `git reset --hard origin/main` to avoid local drift.

### 6. Documentation Standards
- README must include a non-technical User Guide section.
- Explain how to use the UI, what each page does, and how to read results.
- Ensure technical setup and user instructions are both present.
- README must include the environment contract and deployment visibility fields.

### 7. Standard Repo Skeleton (Strict)
- Every repo must follow this root layout. Add missing folders/files if not present.
```
.
├── architecture/
├── docs/
...
### 8. Deployment Visibility Requirement (Strict)
- Every deployment must emit:
  ...
  - Environment
  - Health status
- This metadata must be logged during deploy and captured in CI artifacts.

### 9. Environment Contract (Hard Rule)
- Every project must explicitly declare:
  - Runtime user
  - Ports
  - Volumes
  - External dependencies
  - Restart policy
- This contract must be documented in `README.md` and enforced in deployment scripts.
...
### 4. Styling Philosophy (Frontend)

- Always follow `ui-styling-guide.md` when present.
- Default to clean, neutral, and data-forward UI.
- Favor clarity, spacing, and subtle feedback over decoration.
- Use modern sans-serif fonts and restrained accent colors.
